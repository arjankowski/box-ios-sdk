...
public class RecentItemsManager {
    ...
    public func getRecentItems(queryParams: GetRecentItemsQueryParams = GetRecentItemsQueryParams(), headers: GetRecentItemsHeaders = GetRecentItemsHeaders()) async throws -> RecentItems {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/recent_items")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

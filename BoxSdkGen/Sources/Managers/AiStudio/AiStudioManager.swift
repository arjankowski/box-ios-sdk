...
public class AiStudioManager {
    ...
    public func getAiAgents(queryParams: GetAiAgentsQueryParams = GetAiAgentsQueryParams(), headers: GetAiAgentsHeaders = GetAiAgentsHeaders()) async throws -> AiMultipleAgentResponse {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/ai_agents")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func updateAiAgentById(agentId: String, requestBody: CreateAiAgent, headers: UpdateAiAgentByIdHeaders = UpdateAiAgentByIdHeaders()) async throws -> AiSingleAgentResponseFull {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/ai_agents/")\(agentId)", method: "PUT", headers: headersMap, data: requestBody.serialize(), contentType: "application/json", responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
        return try AiSingleAgentResponseFull.deserialize(from: response.data!)
    }
    ...
    public func getAiAgentById(agentId: String, queryParams: GetAiAgentByIdQueryParams = GetAiAgentByIdQueryParams(), headers: GetAiAgentByIdHeaders = GetAiAgentByIdHeaders()) async throws -> AiSingleAgentResponseFull {
        ...
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/ai_agents/")\(agentId)", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
        return try AiSingleAgentResponseFull.deserialize(from: response.data!)
    }
    ...
    public func deleteAiAgentById(agentId: String, headers: DeleteAiAgentByIdHeaders = DeleteAiAgentByIdHeaders()) async throws {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/ai_agents/")\(agentId)", method: "DELETE", headers: headersMap, responseFormat: ResponseFormat.noContent, auth: auth, networkSession: networkSession))
    }
}

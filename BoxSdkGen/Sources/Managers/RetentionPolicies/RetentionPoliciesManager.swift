...
public class RetentionPoliciesManager {
    ...
    public func getRetentionPolicies(queryParams: GetRetentionPoliciesQueryParams = GetRetentionPoliciesQueryParams(), headers: GetRetentionPoliciesHeaders = GetRetentionPoliciesHeaders()) async throws -> RetentionPolicies {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/retention_policies")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func getRetentionPolicyById(retentionPolicyId: String, queryParams: GetRetentionPolicyByIdQueryParams = GetRetentionPolicyByIdQueryParams(), headers: GetRetentionPolicyByIdHeaders = GetRetentionPolicyByIdHeaders()) async throws -> RetentionPolicy {
        ...
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/retention_policies/")\(retentionPolicyId)", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
        return try RetentionPolicy.deserialize(from: response.data!)
    }
    ...
    public func deleteRetentionPolicyById(retentionPolicyId: String, headers: DeleteRetentionPolicyByIdHeaders = DeleteRetentionPolicyByIdHeaders()) async throws {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/retention_policies/")\(retentionPolicyId)", method: "DELETE", headers: headersMap, responseFormat: ResponseFormat.noContent, auth: auth, networkSession: networkSession))
    }
}

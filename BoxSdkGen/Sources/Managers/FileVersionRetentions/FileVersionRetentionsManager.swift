...
public class FileVersionRetentionsManager {
    ...
    public func getFileVersionRetentions(queryParams: GetFileVersionRetentionsQueryParams = GetFileVersionRetentionsQueryParams(), headers: GetFileVersionRetentionsHeaders = GetFileVersionRetentionsHeaders()) async throws -> FileVersionRetentions {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/file_version_retentions")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func getFileVersionRetentionById(fileVersionRetentionId: String, headers: GetFileVersionRetentionByIdHeaders = GetFileVersionRetentionByIdHeaders()) async throws -> FileVersionRetention {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/file_version_retentions/")\(fileVersionRetentionId)", method: "GET", headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

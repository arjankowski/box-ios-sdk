...
public class StoragePoliciesManager {
    ...
    public func getStoragePolicies(queryParams: GetStoragePoliciesQueryParams = GetStoragePoliciesQueryParams(), headers: GetStoragePoliciesHeaders = GetStoragePoliciesHeaders()) async throws -> StoragePolicies {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/storage_policies")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func getStoragePolicyById(storagePolicyId: String, headers: GetStoragePolicyByIdHeaders = GetStoragePolicyByIdHeaders()) async throws -> StoragePolicy {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/storage_policies/")\(storagePolicyId)", method: "GET", headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

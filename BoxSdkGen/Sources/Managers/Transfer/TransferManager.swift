...
public class TransferManager {
    ...
    public func transferOwnedFolder(userId: String, requestBody: TransferOwnedFolderRequestBody, queryParams: TransferOwnedFolderQueryParams = TransferOwnedFolderQueryParams(), headers: TransferOwnedFolderHeaders = TransferOwnedFolderHeaders()) async throws -> FolderFull {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/users/")\(userId)\("/folders/0")", method: "PUT", params: queryParamsMap, headers: headersMap, data: requestBody.serialize(), contentType: "application/json", responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

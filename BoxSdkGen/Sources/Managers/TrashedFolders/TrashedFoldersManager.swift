...
public class TrashedFoldersManager {
    ...
    public func getTrashedFolderById(folderId: String, queryParams: GetTrashedFolderByIdQueryParams = GetTrashedFolderByIdQueryParams(), headers: GetTrashedFolderByIdHeaders = GetTrashedFolderByIdHeaders()) async throws -> TrashFolder {
        let queryParamsMap: [String: String] = Utils.Dictionary.prepareParams(map: ["fields": Utils.Strings.toString(value: queryParams.fields)])
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/folders/")\(folderId)\("/trash")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func deleteTrashedFolderById(folderId: String, headers: DeleteTrashedFolderByIdHeaders = DeleteTrashedFolderByIdHeaders()) async throws {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/folders/")\(folderId)\("/trash")", method: "DELETE", headers: headersMap, responseFormat: ResponseFormat.noContent, auth: auth, networkSession: networkSession))
    }
}

...
public class WorkflowsManager {
    ...
    public func getWorkflows(queryParams: GetWorkflowsQueryParams, headers: GetWorkflowsHeaders = GetWorkflowsHeaders()) async throws -> Workflows {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/workflows")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

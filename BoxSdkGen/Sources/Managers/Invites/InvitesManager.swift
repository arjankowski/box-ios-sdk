...
public class InvitesManager {
    ...
    public func getInviteById(inviteId: String, queryParams: GetInviteByIdQueryParams = GetInviteByIdQueryParams(), headers: GetInviteByIdHeaders = GetInviteByIdHeaders()) async throws -> Invite {
        let queryParamsMap: [String: String] = Utils.Dictionary.prepareParams(map: ["fields": Utils.Strings.toString(value: queryParams.fields)])
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/invites/")\(inviteId)", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
        return try Invite.deserialize(from: response.data!)
    }
}

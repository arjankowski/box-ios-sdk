...
public class DevicePinnersManager {
    ...
    public func getDevicePinnerById(devicePinnerId: String, headers: GetDevicePinnerByIdHeaders = GetDevicePinnerByIdHeaders()) async throws -> DevicePinner {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/device_pinners/")\(devicePinnerId)", method: "GET", headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func deleteDevicePinnerById(devicePinnerId: String, headers: DeleteDevicePinnerByIdHeaders = DeleteDevicePinnerByIdHeaders()) async throws {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/device_pinners/")\(devicePinnerId)", method: "DELETE", headers: headersMap, responseFormat: ResponseFormat.noContent, auth: auth, networkSession: networkSession))
    }
    ...
    public func getEnterpriseDevicePinners(enterpriseId: String, queryParams: GetEnterpriseDevicePinnersQueryParams = GetEnterpriseDevicePinnersQueryParams(), headers: GetEnterpriseDevicePinnersHeaders = GetEnterpriseDevicePinnersHeaders()) async throws -> DevicePinners {
        ...
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/enterprises/")\(enterpriseId)\("/device_pinners")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

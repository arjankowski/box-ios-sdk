...
public class SignTemplatesManager {
    ...
    public func getSignTemplates(queryParams: GetSignTemplatesQueryParams = GetSignTemplatesQueryParams(), headers: GetSignTemplatesHeaders = GetSignTemplatesHeaders()) async throws -> SignTemplates {
        ...
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/sign_templates")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))
    ...
    public func getSignTemplateById(templateId: String, headers: GetSignTemplateByIdHeaders = GetSignTemplateByIdHeaders()) async throws -> SignTemplate {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await networkSession.networkClient.fetch(options: FetchOptions(url: "\(networkSession.baseUrls.baseUrl)\("/2.0/sign_templates/")\(templateId)", method: "GET", headers: headersMap, responseFormat: ResponseFormat.json, auth: auth, networkSession: networkSession))

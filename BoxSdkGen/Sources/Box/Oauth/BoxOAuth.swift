...
public class BoxOAuth: Authentication {
    ...
    public func getAuthorizeUrl(options: GetAuthorizeUrlOptions = GetAuthorizeUrlOptions()) throws -> String {
        let paramsMap: [String: String] = Utils.Dictionary.prepareParams(map: ["client_id": options.clientId != nil ? options.clientId : config.clientId, "response_type": options.responseType != nil ? options.responseType : "code", "redirect_uri": options.redirectUri, "state": options.state, "scope": options.scope])
        return try "\("https://account.box.com/api/oauth2/authorize?")\(JsonUtils.sdToUrlParams(data: paramsMap.serialize()))"
    }
    ...
    public func getTokensAuthorizationCodeGrant(authorizationCode: String, networkSession: NetworkSession? = nil) async throws -> AccessToken {
        ...
        let token: AccessToken = try await authManager.requestAccessToken(requestBody: PostOAuth2Token(grantType: PostOAuth2TokenGrantTypeField.authorizationCode, clientId: config.clientId, clientSecret: config.clientSecret, code: authorizationCode))
    ...
    public func refreshToken(networkSession: NetworkSession? = nil) async throws -> AccessToken {
        ...
        let authManager: AuthorizationManager = AuthorizationManager(networkSession: networkSession != nil ? networkSession! : NetworkSession())
        let token: AccessToken = try await authManager.requestAccessToken(requestBody: PostOAuth2Token(grantType: PostOAuth2TokenGrantTypeField.refreshToken, clientId: config.clientId, clientSecret: config.clientSecret, refreshToken: tokenUsedForRefresh))
        try await tokenStorage.store(token: token)
        return token
    }
